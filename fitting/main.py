...
def gradcheck(num_trials=100, tolerance=1e-8):
    return gradcheck_helper(affine_transform_loss, num_trials, tolerance)

def gradcheck_helper(loss_fn, num_trials=100, tolerance=1e-8):
    N = 100
    ...
    for _ in range(num_trials):
        ...
        t = np.random.randn(2)
        f_S = lambda _: loss_fn(X, Y, _, t)[0]  # noqa: E731
        f_t = lambda _: loss_fn(X, Y, S, _)[0]  # noqa: E731
        loss, pred, grad_S, grad_t = loss_fn(X, Y, S, t)
        if loss is None:
